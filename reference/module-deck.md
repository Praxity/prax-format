...
Each contiguous lesson compiles to one document. Stable page URLs remain aliases to module fragments. Slides, history, reading seeks and module controls share sequential access rules. The narration reading text is available without generated audio. Recognized nonspoken Soniox delivery cues are omitted from this text, while human sound captions remain. Studio retains the original script for generation and recording freshness. The content keeps the native Praxity layout and authored width/spacing. The optional outline sidebar sits on the left and the optional full-script sidebar on the right, outside the content area. Both start closed by default and share sidebar styling. Authors can choose the initially open panel through `design.deck`. On desktop the reading panel scrolls independently. On narrow or short screens, an open panel replaces the slide area while playback and navigation remain available. Closing the panel returns to the slide and restores focus to its toggle. The slide scrollport spans the available width, including the margins around the authored content. The course outline and script open one at a time. Timing data may drive seeking and highlighting, but timestamps are not displayed in the transcript. The reading panel stops at the first unreleased gate. No separate captions interface is added in this mode.
