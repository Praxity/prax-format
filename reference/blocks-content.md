...
This paragraph and the consecutive paragraphs after it flow beside the image.
```
...
- `wrap` (string): `start | end`. Omit it for the default stacked layout. `start` floats toward the logical inline start and `end` floats toward the logical inline end.
...
Wrapping applies only to a top-level image followed immediately by non-empty prose. The
image must use `width: small | medium | large` and must not set `layout`. Lists, headings,
containers, and later images clear the float. Nested images stay stacked. `alignment` does
not enable wrapping and keeps its existing stacked behavior when `wrap` is omitted.
