...
The first row supplies column headers. An optional Markdown separator row is
accepted and skipped; it never becomes a data row. Table-specific `header`,
`striped`, `highlightHeaderRow`, `highlightHeaderCol`, `gridlines`,
`columnAlignments`, and chart `colors` are retained for older source but currently
have no effect in canonical output. They are not offered as working settings.
